...
mongoose
  .connect(
    "mongodb+srv://capu-pos-admin:MushroomBlueworm118@cluster0.bkkqs.mongodb.net/?retryWrites=true&w=majority",
    {
...
async function sendMessage(message, recipients) {
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    await driver.get(
      `https://facebook.com/messages/t/${recipients[i].user_id}`
    );
    await delay(5000);
...
const getClustersWithMembers = function () {
  return db.Cluster.find().populate("cluster_name").populate("members");
};
...
app.put("/edit-cluster/:id", async (req, res) => {
  ...
  try {
    var edited_cluster = await editCluster(
      { cluster_name: req.body.toEditClusterName },
      req.body.toEditClusterID
    );
    res.status(200).json({
      status: "Success",
      data: {
...
const deleteMember = function (ID) {
  ...
  db.Cluster.updateOne({}, { $pull: { members: { id: ID } } }, { multi: true });
  //delete the member
...
app.post("/submit-posts", async (req, res) => {
  ...
  for (let i = 0; i < postArray.length; i++) {
    urls.push(postArray[i].value);
  }

  try {
    var addedPostList = await addPostList({
      postListName: req.body.postListName,
      urls: urls,
    });
    res.status(201).json({
      status: "Success",
...
app.post("/submit-pages", async (req, res) => {
  ...
  for (let i = 0; i < pageArray.length; i++) {
    urls.push(pageArray[i].value);
  }
  try {
    var addedPageList = await addPageList({
      pageListName: req.body.pageListName,
      urls: urls,
    });
    res.status(201).json({
      status: "Success",
...
//[TAG PEOPLE]
postID =
  "https://www.facebook.com/groups/2196066893899400/posts/2293921350780620";
