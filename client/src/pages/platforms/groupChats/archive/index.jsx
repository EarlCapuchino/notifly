...
import {
  ARCHIVE,
  RESTORE,
} from "../../../../redux/slices/organizations/groupchats";

const paths = [
  {
    name: "Archived Group Chats",
  },
];
export default function GroupChatsArchive() {
  const { theme, maxPage, token, isAdmin } = useSelector(({ auth }) => auth),
    { catalogs, isLoading } = useSelector(({ groupchats }) => groupchats),
    [groupchats, setGroupchats] = useState([]),
  ...
  useEffect(() => {
    if (groupchats.length > 0) {
      let totalPages = Math.floor(groupchats.length / maxPage);
      if (groupchats.length % maxPage > 0) totalPages += 1;
  ...
  }, [groupchats, page, maxPage]);

  useEffect(() => {
    setGroupchats(catalogs);
  }, [catalogs]);
  ...
  return (
    <>
      <BreadCrumb title="Group Chats Archive List" paths={paths} />
