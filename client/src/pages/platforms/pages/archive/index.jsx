...
import { ARCHIVE, RESTORE } from "../../../../redux/slices/organizations/pages";
...
export default function PagesArchive() {
  const { theme, maxPage, token, isAdmin } = useSelector(({ auth }) => auth),
    { catalogs, isLoading } = useSelector(({ pages }) => pages),
  ...
  useEffect(() => {
    if (pages.length > 0) {
      let totalPages = Math.floor(pages.length / maxPage);
      if (pages.length % maxPage > 0) totalPages += 1;
  ...
  return (
    <>
      <BreadCrumb title="Pages Archive List" paths={paths} />
